...
func _on_input_event(viewport: Node, event: InputEvent, _shape_idx: int) -> void:
	if event is InputEventMouseButton:
		...
		if mouse_event.button_index == MOUSE_BUTTON_LEFT:
			if mouse_event.pressed and not _is_dragging and _active_dragger == null:
				...
				if _is_topmost_at_mouse():
					...
					if root_node:
						...
						# Signal the card to request being brought to front
						if root_node.has_method("_on_bring_to_front"):
							root_node._on_bring_to_front()

					# Capture mouse input
					viewport.set_input_as_handled()

			elif not mouse_event.pressed and _is_dragging and _active_dragger == self:
				# Stop dragging
				_is_dragging = false
...
func _input(event: InputEvent) -> void:
	if _is_dragging and _active_dragger == self and event is InputEventMouseMotion:
		if root_node:
...
func _is_topmost_at_mouse() -> bool:
	...
	draggable_areas.sort_custom(func(a, b): 
...
func _on_mouse_exited() -> void:
	if _is_dragging and _active_dragger == self:
		_is_dragging = false
		_active_dragger = null
